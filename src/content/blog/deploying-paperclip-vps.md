...
A reference repo with the Caddyfile, the `docker-compose.yml` overlay, the env template, and a one-shot bootstrap script lives at `paperclipai/examples/vps-deploy`. We rebuild it against each Paperclip release and pin the image tags. Clone, edit two lines, `docker compose up -d`. The walkthrough above is the long-form version of what that repo does in fewer keystrokes.
